        ...
        function(data) {
            ...
            function(data){
                ...
                for (let i = 0; i < data['ticker'].length; i++){
                    better[data['ticker'][i]] = data['weights'][i];
                }
                let get_request = { 'tickers': data['ticker'], 'weights': data['weights']};
                $.getJSON('/api/back_test_portfolio', get_request, function(data) {
                    console.log(data);
                })
                display_both_portfolios(chart_data);
            })
        })
};

function display_both_portfolios(chart_data) {
        document.getElementById("card-page").innerHTML = "<div class=\"card border-primary mb-4 text-center hoverable\" style = \"width: 70%; margin: 0 auto; height: 50%;padding: 20px 0\"><canvas id = 'newCanvas'></canvas>";
        document.getElementById("card-page").setAttribute("style", "width: 95%; background: transparent");
