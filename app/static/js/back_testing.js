...
function card(title, info, description){
    return (
    "<div data-toggle='tooltip' title = '"+description+"' id = 'stats_card' class=\"card bg-light mb-3\" style=\"max-width: 18rem; display: inline-block;\"><div class=\"card-header\">"+title+"</div><div class=\"card-body\"><h5 class=\"card-title\">"+info+"</h5><p class=\"card-text\"></p></div></div>"
    );
}
...
function generate_better(user_id){
    ...
    $.getJSON('/api/back_test_user?user_id='+ user_id, 
        function(data1) {
            ...
            function(data2){
                portfolio_data['sharpe_ratio'] = data2['sharpe_ratio'];
                portfolio_data['ret'] = data2['return'];
                portfolio_data['sigma'] = data2['sigma'];
                portfolio_data['cvar'] = data2['cvar'];
                portfolio_data['var'] = data2['var'];
                ...
                for (header in portfolio_data){
                    switch (header){
                        case "ret":
                            stats.push(card("Average Yearly Return", Math.round(1000000*(portfolio_data[header]*100))/1000000 + "%", "The higher, the better. This tells you how much your portfolio is increasing in value every year."));
                            continue;
                        case "sigma":
                            stats.push(card("Sigma of Portfolio", Math.round(1000000*(portfolio_data[header]))/1000000 + "%", "The standard deviation of this Portfolio. This is a rough average of how much you can deviate from the average return. The higher, the more risky and unpredictable the portfolio."));
                            continue;
                        case "cvar":
                            stats.push(card("CVaR", Math.round(1000000*(portfolio_data[header]))/1000000 + "%", "You have a 1% probability of losing this portion of your portfolio on average. The lower, the better."));
                            continue;
                        case "sharpe_ratio":
                            stats.push(card("Sharpe Ratio", Math.round(1000000*(portfolio_data[header]))/1000000, "A measure that indicates the risk-adjusted rate of return. The higher the better"));
                            continue;
                        case "var":
                            stats.push(card("VaR", Math.round(1000000*(portfolio_data[header]))/1000000 + "%", "You have a 1% probability of losing at least this portion of your portfolio. The lower, the better."));
                            continue;
                        default:
                            continue;
                    }
                ...
                stats.sort();
                stats.unshift(['<h2> Your Portfolio Statistics </h2>'])
                let generated_stats = stats.join('');
                let get_request = {'tickers': data2['ticker'], 'weights': data2['weights']};
                $.getJSON('/api/back_test_portfolio', get_request, function(data3) {
                    passed_data = data3;
                }).done(function(){
                    display_both_portfolios(passed_data, chart_data, better_portfolio, user_id, generated_stats);
                })    
        })
    })
};
...
function display_both_portfolios(new_portfolio_data, chart_data, better, user_id, generated_stats) {
        let user_portfolio = {}
        let asset_prices = {};
        $.getJSON('/api/portfolio_statistics?user_id='+ user_id, function(data){
        ...
        for (header in portfolio_stats){
            switch (header){
                case "ret":
                    stats.push(card("Average Yearly Return", (Math.round(1000000*(portfolio_stats[header]*100)))/1000000 + "%", "The higher, the better. This tells you how much your portfolio is increasing in value every year."));
                    continue;
                case "sigma":
                    stats.push(card("Sigma of Portfolio", (Math.round(1000000*(portfolio_stats[header])))/1000000 + "%", "The standard deviation of this Portfolio. This is a rough average of how much you can deviate from the average return. The higher, the more risky and unpredictable the portfolio."));
                    continue;
                case "cvar":
                    stats.push(card("CVaR", (Math.round(1000000*(portfolio_stats[header])))/1000000 + "%", "You have a 1% probability of losing this portion of your portfolio on average. The lower, the better."));
                    continue;
                case "sharpe_ratio":
                    stats.push(card("Sharpe Ratio", (Math.round(1000000*(portfolio_stats[header])))/1000000, "A measure that indicates the risk-adjusted rate of return. The higher the better"));
                    continue;
                case "var":
                    stats.push(card("VaR", (Math.round(1000000*(portfolio_stats[header])))/1000000 + "%", "You have a 1% probability of losing at least this portion of your portfolio. The lower, the better."));
                    continue;
                default:
                    continue;
            }
        ...
        stats.sort();
        stats.unshift(['<h2> Generated Portfolio Statistics </h2>']);
    
        var table = "<div class = 'tablewrapper'><table class='table'><thead><tr><th scope='col'>Ticker</th><th scope='col'>Value per stock($USD)</th><th scope='col'>Amount in Your Portfolio</th><th scope='col'>Ratio of amounts in generated</th></tr></thead><tbody id = 'bodytable'></tbody></table></div></div>";
        document.getElementById("card-page").innerHTML =  "<div class=\"card border-primary mb-4 text-center hoverable\" style = \"width: 70%; margin: 0 auto; height: 50%;padding: 20px 0\"><div id = 'stats1'></div><div id = 'stats2'></div><h2>Value over time of your portfolio versus generated portfolio</h2><canvas id = 'newCanvas'></canvas><h2>Portfolio Comparison</h2>" + table;
        ...
        document.getElementById('stats2').innerHTML = stats.join('')
        document.getElementById('stats1').innerHTML = generated_stats;
...
function backtest(user_id) {
        ...
        $.getJSON('/api/back_test_portfolio', get_request, function(data) {
            ...
            document.getElementById("card-page").innerHTML = "<div id = 'example'></div><h2> Value of your portfolio over time</h2><canvas id = 'myChart'></canvas>";
            let stats = [];
                for (header in portfolio_stats){
                    switch (header){
                        case "ret":
                ...
                stats.unshift(['<h2> Your Portfolio Statistics </h2>'])
                let generated_stats = stats.join('');
            document.getElementById('example').innerHTML = generated_stats;
